...
export default function Create({ auth, categories }) {
    ...
    return (
        <AuthenticatedLayout auth={auth}>
            <Head title="Add Store Item" />
            <div className="max-w-lg mx-auto">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">Add Store Item</h2>
                <form onSubmit={submit} className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Minimum Stock (optional)</label>
                        <input type="number" min="0" value={data.minimum_stock} onChange={e => setData('minimum_stock', e.target.value)}
                            className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-brand-400" placeholder="Alert when stock reaches this level" />
                    </div>
