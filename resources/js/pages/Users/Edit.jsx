...
export default function Edit({ auth, user, departments, roles, deptHeads }) {
    const { t } = useLang();
    const { data, setData, post, processing, errors } = useForm({
        ...
        dept_head_id: user.dept_head_id ?? '',
    });
    ...
    return (
        <AuthenticatedLayout auth={auth}>
            <Head title={`${t('usr.form_edit')} ${user.name}`} />
            <div className="max-w-lg mx-auto">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">{t('usr.form_edit')}</h2>
                <form onSubmit={submit} className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('usr.new_pw')}</label>
                        <input type="password" value={data.password} onChange={e => setData('password', e.target.value)} className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-brand-400" minLength={8} />
                    </div>
                    ...
                    <button type="submit" disabled={processing} className="w-full bg-brand-400 text-white rounded-xl py-2.5 text-sm font-medium hover:bg-brand-600 disabled:opacity-50">
                        {processing ? t('usr.saving') : t('usr.save_changes')}
                    </button>
                </form>
            </div>
        </AuthenticatedLayout>
