...
export default function Index({ auth, tasks }) {
    ...
    const statusLabel = (s) => s === 'in_progress' ? t('task.in_progress') : s === 'pending_check' ? t('task.pending_check') : t('task.assigned');
    return (
        <AuthenticatedLayout auth={auth}>
            <Head title={t('task.title')} />
            <div className="space-y-4">
                <h2 className="text-xl font-semibold text-gray-900">{t('task.title')}</h2>

                {tasks.length === 0 ? (
                    <div className="bg-white rounded-2xl shadow-sm p-8 text-center text-gray-400">{t('task.empty')}</div>
                ) : (
                                        ...
                                        <div className="flex items-start justify-between">
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-500 mt-0.5">
                                                    {wo.asset?.name ?? t('task.no_asset')} · {assignment?.scheduled_date ? new Date(assignment.scheduled_date).toLocaleDateString(locale) : ''} · {assignment?.shift}
                                                </p>
                                            </div>
