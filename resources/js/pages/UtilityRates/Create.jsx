...
export default function Create({ auth }) {
    ...
    return (
        <AuthenticatedLayout auth={auth}>
            <Head title={t('ur.form_create')} />
            <div className="max-w-lg mx-auto">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">{t('ur.form_create')}</h2>
                <form onSubmit={submit} className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" step="1" min="0" value={data.cost_per_unit} onChange={e => setData('cost_per_unit', e.target.value)} className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-brand-400" placeholder={t('ur.cost_ph')} required />
                    ...
                    <div>
                        ...
                        <textarea value={data.notes} onChange={e => setData('notes', e.target.value)} rows={2} className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-brand-400" placeholder={t('ur.notes_ph')} />
                    </div>
                    ...
                    <button type="submit" disabled={processing} className="w-full bg-brand-400 text-white rounded-xl py-2.5 text-sm font-medium hover:bg-brand-600 disabled:opacity-50">
                        {processing ? t('ur.saving') : t('ur.save')}
                    </button>
                </form>
            </div>
        </AuthenticatedLayout>
