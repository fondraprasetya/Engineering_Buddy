...
export default function Edit({ auth, location, buildings, areas }) {
    ...
    return (
        <AuthenticatedLayout auth={auth}>
            <Head title={`${t('loc.edit_title')} ${location.name}`} />
            <div className="max-w-lg mx-auto">
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-semibold text-gray-900">{t('loc.edit_title')}</h2>
                    <Link href="/locations" className="text-sm text-brand-600 hover:text-brand-700 font-medium">{t('loc.back')}</Link>
                </div>
                <form onSubmit={submit} className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('loc.code')}</label>
                    ...
                    <button type="submit" disabled={processing} className="w-full bg-brand-400 text-white rounded-xl py-2.5 text-sm font-medium hover:bg-brand-600 disabled:opacity-50">
                        {processing ? t('loc.saving') : t('loc.save_changes')}
                    </button>
                </form>
            </div>
        </AuthenticatedLayout>
