...
export default function Create({ auth, postAccounts, workOrders, projects, schedules }) {
    const { t } = useLang();
    const { data, setData, post, processing, errors } = useForm({
        expense_date: '', post_account: '', amount: '', description: '',
        document_ref: '', status: 'actual',
    ...
    return (
        <AuthenticatedLayout auth={auth}>
            <Head title={t('exp.record')} />
            <div className="max-w-lg mx-auto">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">{t('exp.form_create')}</h2>
                <form onSubmit={submit} className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('exp.date')}</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('exp.amount')}</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('exp.doc_ref')}</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('exp.status')}</label>
                        <div className="flex gap-3">
                            <label className="flex items-center gap-2 cursor-pointer">
                                ...
                                <span className="text-sm text-gray-700">{t('exp.opt_actual')}</span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer">
                                ...
                                <span className="text-sm text-gray-700">{t('exp.opt_accrue')}</span>
                            </label>
                        </div>
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('exp.linked_to')}</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('exp.desc_opt')}</label>
                    ...
                    <button type="submit" disabled={processing} className="w-full bg-brand-400 text-white rounded-xl py-2.5 text-sm font-medium hover:bg-brand-600 disabled:opacity-50">
                        {processing ? t('exp.saving') : t('exp.save')}
                    </button>
                </form>
            </div>
        </AuthenticatedLayout>
